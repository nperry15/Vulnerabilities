...
module Access
	...
	def authenticate(user, passwd)
		if get_user(user) then
			...
			pass = @db.execute(query,[user])

			if pass.nil?
				return nil
			else
				pass_string = pass.join('')
				if pass_string == passwd
...
module User
        ...
        def search(user_query)
                users = []
				sanitized = san_html(user_query)

				query = %{
                SELECT User, Avatar, Description
                FROM Users
                WHERE User LIKE '%#{sanitized}%' OR
		      Description LIKE '%#{sanitized}%'
                }
	...
	def register(user, filename, file, password, confirm)
		user = san_html(user)
		user = san_sql(user)
		filename = san_html(filename)
		filename = san_sql(filename)
		password = san_html(password)
		password = san_sql(password)
		confirm = san_html(confirm)
		confirm = san_sql(confirm)

		return false if not get_user(user).nil?
	...
	def update_prefs(user, session, description, token)
		if  (authorize(user, session) && @tokens[user] == token)
			description = san_html(description)
			query = %{
			UPDATE Users
			SET Description = ?
			WHERE User = ?
